...
<script>
	import nvMenu from '../../components/menu/menu.vue'
	...
	export default{
		...
		mounted(){
			this.$nextTick(()=>{
				// 获取数据
				axios.get('/record.json')
				  	.then((res)=>{
				  		var resData=res.data;
				  		var switchRecordData=this.switchRecordDataFun(resData.recordListData)
				  		this.resData.recordListData=switchRecordData
				  	})
				  	.catch((error)=>{
